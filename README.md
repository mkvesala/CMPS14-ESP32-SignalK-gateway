...
- **Fresh client per reconnect:** `SignalKBroker` owns the `WebsocketsClient` via `std::unique_ptr`; each reconnect builds a brand-new client and destroys the previous one, so every attempt starts from a clean TCP / lwIP socket state. This prevents a stuck underlying socket from making all future reconnects fail permanently until a reboot — a failure mode that could appear only after prolonged uptime (~12–48 h) even while WiFi, heap and the main loop stayed healthy.
- **Watchdogs (last-resort fallback):** the network watchdog (`ESP.restart()` after ~10 min of a silent-but-associated link) and the loop-runtime watchdog (restart if the loop runtime EMA stays degraded) remain in place as a safety net. Individual iteration times are clamped (`LOOP_SAMPLE_CAP_US`) before entering the EMA, so a single blocking call — a `ws.connect()` against an unreachable host, the WiFi teardown delay — cannot trip the watchdog on its own; the unclamped peak is still reported on the LCD and in `/status` (`runtime_peak`) for diagnostics. With ping/pong liveness handling the common case, both watchdogs are expected to stay dormant and may be removed in a future release once liveness is field-proven.
- **Restart cause is preserved:** whichever watchdog fires — and likewise a restart from the web UI — records its reason to NVS before rebooting, so `/status` reports it as `last_reset` after the device comes back. The stored reason is consumed on boot, so it always describes only the previous restart. Crashes that never reach the NVS write (panic, brownout) are covered by the separate `esp_reset` field, taken from ESP-IDF's `esp_reset_reason()`.
